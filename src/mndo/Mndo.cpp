...
void Mndo::SetMessages(){
   ...
   this->errorMessageCalcZMatrixForceEtaNull 
      = "Error in mndo::Mndo::CalcZMatrixForce: Mndo::etaMatrixForce is NULL. Call Mndo::CalcEtaMatrixForce before calling Mndo::CalcZMatrixForce.\n";
   this->errorMessageCalcHessian
      = "Error in mndo::Mndo::CalcHessian::Conditions for calculation are wrong.\n";
   this->errorMessageGetFockDiagElementDistri
      = "Error in mndo::Mndo::GetFockDiagElement: Distributed storing two electron integrals on multiple nodes are not implemented.\n";
   this->errorMessageGetFockOffDiagElementDistri
      = "Error in mndo::Mndo::GetFockOffDiagElement: Distributed storing two electron integrals on multiple nodes are not implemented.\n";
...
double Mndo::GetDiatomCoreRepulsionEnergy(const Atom& atomA, const Atom& atomB) const{
   ...
   double tetacValue = 0.0;
   if(this->twoElecInt==OnNode){
      tetacValue = this->twoElecsTwoAtomCores[atomA.GetIndex()][atomB.GetIndex()][s][s][s][s];
   }
   else if(this->twoElecInt==Direct){
      tetacValue = this->GetNddoRepulsionIntegral(atomA, s, s, atomB, s, s);
   }
...
double Mndo::GetDiatomCoreRepulsion1stDerivative(const Atom& atomA, const Atom& atomB, 
   ...
   double tetacValue1stDeriv = this->GetNddoRepulsionIntegral1stDerivative(
   ...
   value = atomA.GetCoreCharge()*atomB.GetCoreCharge()
          *(tetacValue1stDeriv*tmp + tetacValue*tmpDeriv); 
   return value;
}
...
double Mndo::GetDiatomCoreRepulsion2ndDerivative(const Atom& atomA,
   ...
   double tetacValue1stDeriv1 = this->GetNddoRepulsionIntegral1stDerivative(atomA, s, s, 
                                                                            atomB, s, s, 
                                                                            axisA1);
   double tetacValue1stDeriv2 = this->GetNddoRepulsionIntegral1stDerivative(atomA, s, s, 
                                                                            atomB, s, s, 
                                                                            axisA2);
   double tetacValue2ndDeriv = this->GetNddoRepulsionIntegral2ndDerivative(atomA, s, s, 
                                                                           atomB, s, s, 
                                                                           axisA1, 
                                                                           axisA2);
   ...
   value = atomA.GetCoreCharge()*atomB.GetCoreCharge();
   value *= tetacValue*tmp2ndDeriv 
           +tetacValue1stDeriv1*tmp1stDeriv2 
           +tetacValue1stDeriv2*tmp1stDeriv1
           +tetacValue2ndDeriv*tmp;
   return value;
}
...
double Mndo::GetFockDiagElement(const Atom& atomA, 
                                ...
                                bool isGuess) const{
   if(this->twoElecInt==Distri){
      stringstream ss;
      ss << this->errorMessageGetFockDiagElementDistri;
   ...
   if(!isGuess){
      ...
      try{
         ...
         for(int nu=0; nu<atomA.GetValenceSize(); nu++){
            ...
            tmp += orbitalElectronPopulation[nu+firstAOIndexA]
                                            [nu+firstAOIndexA]
         ...
         for(int B=0; B<totalNumberAtoms; B++){
            if(B != indexAtomA){
               ...
               double const* const* const* const* diaTetac = NULL;
               if(this->twoElecInt==OnNode){
                  diaTetac = twoElecsTwoAtomCores[indexAtomA][B];
               }
               if(this->twoElecInt==Direct){
                  this->CalcDiatomicTwoElecsTwoCores(diatomicTwoElecsTwoCores, 
                                                     ...
                                                     tmpVectorBC, 
                                                     indexAtomA, B);
                  diaTetac = diatomicTwoElecsTwoCores;
               }
               for(int lambda=0; lambda<valenceSizeB; lambda++){
                  for(int sigma=0; sigma<valenceSizeB; sigma++){
                     tmp2 += orbitalElectronPopulation[lambda+firstAOIndexB]
                                                      [sigma+firstAOIndexB]
                            *diaTetac[mu][mu][lambda][sigma];
                  }
               }
               tmp2 += this->GetElectronCoreAttraction(indexAtomA, 
                                                       B, 
                                                       mu, 
                                                       mu, 
                                                       diaTetac);
...
double Mndo::GetFockOffDiagElement(const Atom& atomA, 
   ...
   if(this->twoElecInt==Distri){
      stringstream ss;
      ss << this->errorMessageGetFockOffDiagElementDistri;
      throw MolDSException(ss.str());
   }
   double value = 0.0;
   int firstAOIndexA = atomA.GetFirstAOIndex();
   ...
   if(isGuess){
      ...
      try{
         ...
         if(indexAtomA == indexAtomB){
            ...
            double tmp = (1.5*exchange - 0.5*coulomb)
            ...
            for(int BB=0; BB<totalNumberAtoms; BB++){
               if(BB != indexAtomA){
                  ...
                  double const* const* const* const* diaTetac = NULL;
                  if(this->twoElecInt==OnNode){
                     diaTetac = twoElecsTwoAtomCores[indexAtomA][BB];
                  }
                  if(this->twoElecInt==Direct){
                     this->CalcDiatomicTwoElecsTwoCores(diatomicTwoElecsTwoCores, 
                                                        tmpDiatomicTwoElecsTwoCores,
                  ...
                  for(int lambda=0; lambda<valenceSizeBB; lambda++){
                     for(int sigma=0; sigma<valenceSizeBB; sigma++){
                        tmp += orbitalElectronPopulation[lambda+firstAOIndexBB]
                                                        [sigma+firstAOIndexBB]
                              *diaTetac[mu][nu][lambda][sigma];
                     }
                  }
                  tmp += this->GetElectronCoreAttraction(indexAtomA, 
                                                         BB, 
                                                         mu, 
                                                         nu, 
                                                         diaTetac);
            ...
            double tmp = bondParameter*overlapAOs[mu+firstAOIndexA][nu+firstAOIndexB];
            for(int sigma=0; sigma<atomA.GetValenceSize(); sigma++){
               int valenceSizeB = atomB.GetValenceSize();
               double const* const* const* const* diaTetac = NULL;
               if(this->twoElecInt==OnNode){
                  diaTetac = twoElecsTwoAtomCores[indexAtomA][indexAtomB];
               }
               if(this->twoElecInt==Direct){
                  this->CalcDiatomicTwoElecsTwoCores(diatomicTwoElecsTwoCores, 
                                                     tmpDiatomicTwoElecsTwoCores,
               ...
               for(int lambda=0; lambda<valenceSizeB; lambda++){
                  tmp -= 0.5*orbitalElectronPopulation[sigma+firstAOIndexA]
                                                      [lambda+firstAOIndexB]
                        *diaTetac[mu][sigma][nu][lambda];
               }
            }
            value += tmp;
         }
...
double Mndo::GetElectronCoreAttraction(int indexAtomA, 
   ...
   return -1.0*atomB.GetCoreCharge()*diatomicTwoElecsTwoAtomCores[mu][nu][s][s];
}
                  ...
                  for(int nu=mu; nu<=lastAOIndexA; nu++){
                     for(int lambda=firstAOIndexB; lambda<=lastAOIndexB; lambda++){
                        for(int sigma=lambda; sigma<=lastAOIndexB; sigma++){
                           double**** diaTetac = NULL;
                           if(this->twoElecInt==OnNode){
                              diaTetac = twoElecsTwoAtomCores[A][B];
                           }
                           if(this->twoElecInt==Direct){
                              this->CalcDiatomicTwoElecsTwoCores(diatomicTwoElecsTwoCores, 
                                                                 tmpDiatomicTwoElecsTwoCores,
                           ...
                           gamma = diaTetac[mu-firstAOIndexA]
                                           [nu-firstAOIndexA]
                                           [lambda-firstAOIndexB]
                                           [sigma-firstAOIndexB];
...
void Mndo::CalcCISMatrix(double** matrixCIS) const{
   ...
   // Fast and parallelized algorith, but this is not easy to read. 
   // Slow algorithm (not parallelized) is alos written below.
               ...
               for(int l=k; l<this->matrixCISdimension; l++){
                  ...
                  for(int A=0; A<molecule->GetAtomVect().size(); A++){
                     ...
                     for(int B=A; B<molecule->GetAtomVect().size(); B++){
                        ...
                        double gamma = 0.0;
                        if(A!=B){
                           double**** gamma_AB = NULL;
                           if(this->twoElecInt==OnNode){
                              gamma_AB = this->twoElecsTwoAtomCores[A][B];
                           }
                           if(this->twoElecInt==Direct){
                              this->CalcDiatomicTwoElecsTwoCores(diatomicTwoElecsTwoCores, 
                                                                 tmpDiatomicTwoElecsTwoCores,
                                                                 ...
                                                                 tmpVectorBC, 
                                                                 A, B);
                              gamma_AB = diatomicTwoElecsTwoCores;
                           }
                           for(int mu=firstAOIndexA; mu<=lastAOIndexA; mu++){
   ...
   // End of the fast and parallelized algorith.
...
void Mndo::MallocTmpMatricesEachThreadCalcHessianSCF(double*****    diatomicOverlapAOs1stDerivs,
...
void Mndo::FreeTmpMatricesEachThreadCalcHessianSCF(double*****    diatomicOverlapAOs1stDerivs,
                                                   double******   diatomicOverlapAOs2ndDerivs,
                                                   double*******  diatomicTwoElecsTwoCores1stDerivs,
                                                   double******** diatomicTwoElecsTwoCores2ndDerivs,
                                                   double***      tmpRotMat,
...
#pragma omp parallel
      {
         ...
         try{
            this->MallocTmpMatricesEachThreadCalcHessianSCF(&diatomicOverlapAOs1stDerivs,
...
#pragma omp critical
         ...
         this->FreeTmpMatricesEachThreadCalcHessianSCF(&diatomicOverlapAOs1stDerivs,
                                                       &diatomicOverlapAOs2ndDerivs,
                                                       &diatomicTwoElecsTwoCores1stDerivs, 
                                                       &diatomicTwoElecsTwoCores2ndDerivs,
                                                       &tmpRotMat,
...
void Mndo::SolveCPHF(double** solutionsCPHF,
   ...
   try{
      this->MallocTmpMatricesSolveCPHF(&matrixCPHF, dimensionCPHF);
      ...
      this->FreeTmpMatricesSolveCPHF(&matrixCPHF, dimensionCPHF);
      throw ex;
   }
   this->FreeTmpMatricesSolveCPHF(&matrixCPHF, dimensionCPHF);
}
...
void Mndo::CalcStaticFirstOrderFock(double* staticFirstOrderFock,
   ...
   double*   tmpVectorBC                        = NULL;
   try{
      this->MallocTmpMatricesStaticFirstOrderFock(&diatomicTwoElecsTwoCores1stDerivs, 
                                                  &diatomicOverlapAOs1stDerivs,
                                                  &tmpRotMat,
                                                  &tmpRotMat1stDerivs,
                                                  &tmpDiatomicTwoElecsTwoCores);
      ...
      this->FreeTmpMatricesStaticFirstOrderFock(&diatomicTwoElecsTwoCores1stDerivs, 
                                                &diatomicOverlapAOs1stDerivs,
                                                &tmpRotMat,
                                                &tmpRotMat1stDerivs,
                                                &tmpDiatomicTwoElecsTwoCores);
...
void Mndo::MallocTmpMatricesStaticFirstOrderFock(double****** diatomicTwoElecsTwoCores1stDeriv,
...
void Mndo::FreeTmpMatricesStaticFirstOrderFock(double****** diatomicTwoElecsTwoCores1stDeriv,
                                               double****   diatomicOverlapAOs1stDeriv,
                                               double***    tmpRotMat,
                                               double****   tmpRotMat1stDerivs,
                                               double*****  tmpDiatomicTwoElecsTwoCores)const{
...
void Mndo::MallocTmpMatricesSolveCPHF(double*** matrixCPHF,
                                      int dimensionCPHF) const{
   MallocerFreer::GetInstance()->Malloc<double>(matrixCPHF, dimensionCPHF, dimensionCPHF);
}

void Mndo::FreeTmpMatricesSolveCPHF(double*** matrixCPHF,
                                    int dimensionCPHF) const{
...
#pragma omp parallel
            {
               ...
               try{
                  this->MallocTmpMatricesCalcForce(&diatomicOverlapAOs1stDerivs, 
                                                   &diatomicTwoElecsTwoCores1stDerivs,
                                                   &tmpDiaOverlapAOsInDiaFrame,
                                                   &tmpDiaOverlapAOs1stDerivInDiaFrame,
                                                   &tmpRotMat,
...
#pragma omp critical
               ...
               this->FreeTmpMatricesCalcForce(&diatomicOverlapAOs1stDerivs, 
                                              &diatomicTwoElecsTwoCores1stDerivs,
                                              &tmpDiaOverlapAOsInDiaFrame,
                                              &tmpDiaOverlapAOs1stDerivInDiaFrame,
                                              &tmpRotMat,
      ...
      MallocerFreer::GetInstance()->Free<double>(&comMatrixForce, elecStates.size(), this->molecule->GetAtomVect().size(), CartesianType_end);
      throw ex;
   }
   MallocerFreer::GetInstance()->Free<bool>(&forceAdded, this->molecule->GetAtomVect().size());
   MallocerFreer::GetInstance()->Free<double>(&comMatrixForce, elecStates.size(), this->molecule->GetAtomVect().size(), CartesianType_end);
   ...
   int num = elecStates.size()*this->molecule->GetAtomVect().size()*CartesianType_end;
   MolDS_mpi::MpiProcess::GetInstance()->Broadcast(&this->matrixForce[0][0][0], num, mpiHeadRank);

}

void Mndo::MallocTmpMatricesCalcForce(double****   diatomicOverlapAOs1stDerivs, 
                                      double****** diatomicTwoElecsTwoCores1stDerivs,
                                      double***    tmpDiaOverlapAOsInDiaFrame,
                                      double***    tmpDiaOverlapAOs1stDerivInDiaFrame,
                                      double***    tmpRotMat,
...
void Mndo::FreeTmpMatricesCalcForce(double****   diatomicOverlapAOs1stDerivs, 
                                    double****** diatomicTwoElecsTwoCores1stDerivs,
                                    double***    tmpDiaOverlapAOsInDiaFrame,
                                    double***    tmpDiaOverlapAOs1stDerivInDiaFrame,
                                    double***    tmpRotMat,
                  ...
                  for(int nu=mu; nu<=lastAOIndexA; nu++){
                     for(int lambda=firstAOIndexB; lambda<=lastAOIndexB; lambda++){
                        for(int sigma=lambda; sigma<=lastAOIndexB; sigma++){
                           double**** diaTetac = NULL;
                           if(this->twoElecInt==OnNode){
                              diaTetac = this->twoElecsTwoAtomCores[A][B];
                           }
                           if(this->twoElecInt==Direct){
                              this->CalcDiatomicTwoElecsTwoCores(diatomicTwoElecsTwoCores, 
                                                                 tmpDiatomicTwoElecsTwoCores,
                           ...
                           double tetacValue = diaTetac[mu-firstAOIndexA]
                                                       [nu-firstAOIndexA]
                                                       [lambda-firstAOIndexB]
                                                       [sigma-firstAOIndexB];
                           double temp = 0.0;
                           ...
                           if(lambda!=sigma){
                              if(isMoPOcc){
                                 ...
                                 value += tetacValue*fockMatrix_moI[sigma]*temp;
                              }
                              else{
                                 int p = moP - numberOcc;
                           ...
                           if(mu!=nu){
                              if(isMoPOcc){
                                 ...
                                 value += tetacValue*fockMatrix_moI[nu]*temp;
                     ...
                     for(int lambda=firstAOIndexB; lambda<=lastAOIndexB; lambda++){
                        for(int sigma=firstAOIndexB; sigma<=lastAOIndexB; sigma++){
                           ...
                           if(mu==nu && lambda==sigma){
                              OrbitalType orbitalMu = atomA.GetValence(mu-firstAOIndexA);
                              OrbitalType orbitalLambda = atomB.GetValence(lambda-firstAOIndexB);
                              tetacValue = this->GetCoulombInt(orbitalMu, 
                                                               orbitalLambda, 
                                                               atomA);
                           }
                           else if((mu==lambda && nu==sigma) || (nu==lambda && mu==sigma) ){
                              OrbitalType orbitalMu = atomA.GetValence(mu-firstAOIndexA);
                              OrbitalType orbitalNu = atomA.GetValence(nu-firstAOIndexA);
                              tetacValue = this->GetExchangeInt(orbitalMu, 
                                                                orbitalNu, 
                                                                atomA);
                           }
                           else{
                  ...
                  for(int lambda=firstAOIndexB; lambda<=lastAOIndexB; lambda++){
                     ...
                     for(int sigma=lambda; sigma<=lastAOIndexB; sigma++){
                        ...
                        if(A!=B){
                           ...
                           gamma = diaTetac[muOffSet][nuOffSet][lambdaOffSet][sigmaOffSet];
                        }
                        else{
                           if(mu==nu && lambda==sigma){
...
void Mndo::MallocTmpMatricesCalcTwoElecsTwoAtomCores(double***** diatomicTwoElecsTwoCores, 
                                                     double**    tmpDiatomicTwoElecsTwoCores,
                                                     double***   tmpRotMat,
                                                     double***   tmpMatrixBC,
                                                     double**    tmpVectorBC) const{
...
void Mndo::FreeTmpMatricesCalcTwoElecsTwoAtomCores(double***** diatomicTwoElecsTwoCores, 
                                                   double**    tmpDiatomicTwoElecsTwoCores,
                                                   double***   tmpRotMat,
                                                   double***   tmpMatrixBC,
                                                   double**    tmpVectorBC) const{
...
void Mndo::CalcDiatomicTwoElecsTwoCores(double**** matrix, 
   ...
   if(ssssOnly){
      matrix[s][s][s][s] = this->GetNddoRepulsionIntegral(atomA, s, s, atomB, s, s);
   }
   else{
...
void Mndo::CalcDiatomicTwoElecsTwoCores(double**** matrix, 
                                        ...
                                        const Atom& atomB) const{
   bool ssssOnly=false;
   this->CalcDiatomicTwoElecsTwoCores(matrix, tmpVec, tmpRotMat, tmpMatrixBC, tmpVectorBC, atomA, atomB, ssssOnly);
}
...
void Mndo::CalcDiatomicTwoElecsTwoCores(double**** matrix, 
   ...
   const Atom& atomA = *this->molecule->GetAtomVect()[indexAtomA];
   const Atom& atomB = *this->molecule->GetAtomVect()[indexAtomB];
   this->CalcDiatomicTwoElecsTwoCores(matrix,  tmpVec, tmpRotMat,  tmpMatrixBC, tmpVectorBC, atomA,  atomB);
}
...
void Mndo::RotateDiatomicTwoElecsTwoCores1stDerivativesToSpaceFrame(
   ...
   try{
      this->MallocTmpMatricesRotateDiatomicTwoElecsTwoCores1stDerivs(&twiceRotatingMatrix,
      ...
      this->FreeTmpMatricesRotateDiatomicTwoElecsTwoCores1stDerivs(&twiceRotatingMatrix,
                                                                   &twiceRotatingMatrixDerivA,
                                                                   &twiceRotatingMatrixDerivB,
                                                                   &oldMatrix,
                                                                   &rotatedMatrix,
...
void Mndo::MallocTmpMatricesRotateDiatomicTwoElecsTwoCores1stDerivs(double*** twiceRotatingMatrix,
...
void Mndo::FreeTmpMatricesRotateDiatomicTwoElecsTwoCores1stDerivs(double*** twiceRotatingMatrix,
                                                                  double*** twiceRotatingMatrixDerivA,
                                                                  double*** twiceRotatingMatrixDerivB,
                                                                  double*** oldMatrix,
                                                                  double*** rotatedMatrix,
